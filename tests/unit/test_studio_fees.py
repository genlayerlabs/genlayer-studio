...
async def test_sim_estimate_transaction_fees_preserves_caller_fee_envelope(
    ...
    assert result["recommendedPreset"] == {"messageAllocations": [allocation]}
...
async def test_sim_estimate_transaction_fees_returns_mode2_recommended_preset(
    ...
    assert result["scenario"] == "mode-2-message"
    assert result["feeAccounting"]["message_fee_consumed"] == message_budget
    ...
    assert report["genvmBuckets"]["message"] == message_budget
    assert (
    ...
    assert report["messageFees"]["declaredConsumed"] == message_budget
    assert report["messageFees"]["remaining"] == 0
    assert message["messageFeeMode"] == "mode2"
    assert message["declaredBudget"] == message_budget
    ...
    assert preset["observed"]["messageFeeBudget"] == message_budget
...
async def test_sim_estimate_transaction_fees_returns_mode1_observed_message_preset(
    ...
    assert report["messageFees"]["budget"] == message_budget
    assert report["messageFees"]["declaredConsumed"] == message_budget
    assert report["messageFees"]["reportedTotal"] == message_budget
    ...
    assert preset["distribution"]["totalMessageFees"] == expected_padded_message_budget
    assert preset["observed"]["messageFeeBudget"] == message_budget
    assert preset["observed"]["declaredMessageFees"] == message_budget
