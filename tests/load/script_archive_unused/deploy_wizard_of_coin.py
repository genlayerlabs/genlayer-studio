...
CONTRACT_PATH = os.path.join(
    os.path.dirname(__file__), "../../examples/contracts/wizard_of_coin.py"
)
...
def get_client():
    ...
    client = create_client(chain=localnet, endpoint=BASE_URL, account=account)
...
def deploy_wizard_contract(client):
    ...
    try:
        ...
        tx_hash = client.deploy_contract(code=contract_code, kwargs={"have_coin": True})
        ...
        if receipt.status != 1:
            print_status(
                f"❌ Deployment failed with status: {receipt.status}", Colors.RED
            )
            return None
        ...
        try:
            ...
            if "result" in tx_data and tx_data["result"]:
                ...
                if not contract_address:
                    contract_address = raw_tx.get("contractAddress") or raw_tx.get(
                        "creates"
                    )
                ...
                if contract_address:
                    print_status(
                        f"✅ Found contract address: {contract_address}", Colors.GREEN
                    )
                else:
                    print_status(
                        "⚠️  Contract address not found in transaction data",
                        Colors.YELLOW,
                    )
                    print_status(
                        f"Transaction data keys: {list(raw_tx.keys())}", Colors.CYAN
                    )
        ...
        print_status(
            f"✅ WizardOfCoin contract deployed at: {contract_address}", Colors.GREEN
        )
...
def test_write_simulation(client, contract_address):
    ...
    try:
        ...
        if "SystemError: 6: forbidden" in error_str and "storage.py" in error_str:
            print_status(
                "🎯 FOUND: SystemError: 6: forbidden in storage.py!", Colors.YELLOW
            )
...
def test_state_persistence(client, contract_address, initial_state):
    ...
    try:
        ...
        if initial_state == final_state:
            print_status(
                "🎯 CONFIRMED: Write simulation did NOT modify actual contract state!",
                Colors.GREEN,
            )
            print_status("State remains unchanged after simulation.", Colors.GREEN)
        else:
            print_status(
                "⚠️  WARNING: Contract state was modified by write simulation!",
                Colors.YELLOW,
            )
...
def test_actual_write(client, contract_address):
    ...
    try:
        ...
        if receipt.status == 1:
            ...
            print_status(
                f"⚠️  Transaction failed with status: {receipt.status}", Colors.YELLOW
            )
...
def verify_final_state(client, contract_address, initial_state):
    ...
    try:
        ...
        if initial_state != final_state:
            print_status(
                "✅ State changed! The wizard gave away the coin!", Colors.GREEN
            )
        else:
...
def main():
    ...
    try:
        ...
        with open(
            os.path.join(os.path.dirname(__file__), ".last_deployed_contract"), "w"
        ) as f:
        ...
        if simulation_result == "ERROR_REPRODUCED":
            ...
            print_status(
                "✅ SystemError: 6: forbidden in storage.py detected", Colors.YELLOW
            )
        elif simulation_result:
            print_status(
                "✅ Write simulation operations tested (no error detected)",
                Colors.GREEN,
            )
        else:
