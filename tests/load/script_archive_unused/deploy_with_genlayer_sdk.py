...
class GenLayerClient:
    ...
    def deploy_contract(self, contract_code, constructor_args=None):
        ...
        print(f"Contract schema validated: {json.dumps(schema, indent=2)}", file=sys.stderr)
        ...
        print(f"Constructor calldata (base64): {base64.b64encode(calldata).decode()}", file=sys.stderr)
        ...
        transaction = {
            ...
            'to': to_checksum_address("0xb7278a61aa25c888815afc32ad3cc52ff24fe575"),  # ConsensusMain
            'value': 0,
            'data': tx_data,
    ...
    def _encode_add_transaction(self, sender, recipient, num_validators, max_rotations, data):
        ...
        try:
            ...
            if receipt.get('logs') and len(receipt['logs']) > 0:
                for log in receipt['logs']:
                    ...
                    if 'address' in log and log['address'] != "0xb7278A61aa25c888815aFC32Ad3cC52fF24fE575":
            ...
            print(f"⚠️  Transaction not confirmed yet. Check manually with hash: {tx_hash}")
