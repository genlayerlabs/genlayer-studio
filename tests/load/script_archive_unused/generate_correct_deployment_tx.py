...
def main():
    ...
    if account.address.lower() != expected_address.lower():
        print(f"Warning: Address mismatch! Expected {expected_address}, got {account.address}", file=sys.stderr)
    ...
    if len(decoded_deployment) >= 2:
        ...
        print(f"Calldata portion (base64): {base64.b64encode(calldata_portion).decode()}", file=sys.stderr)
    ...
    transaction = {
        ...
        'value': 0,
        'data': tx_data,
        'chainId': 61999  # GenLayer chain ID (0xf22f)
    }
