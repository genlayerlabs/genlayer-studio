...
#     echo "Warning: Could not get transaction hash from deployment"
#     echo "Response: $DEPLOY_RESPONSE"
#     # Use a placeholder for load testing
#     CONTRACT_ADDRESS="0x0000000000000000000000000000000000000000"
# else
#     echo "Deployment transaction: $TX_HASH"
#     # Try to get receipt to extract contract address
#     sleep 2  # Give transaction time to be processed
#     RECEIPT_RESPONSE=$(curl -s -X POST $BASE_URL \
#         -H "Content-Type: application/json" \
#         -d "{\"jsonrpc\":\"2.0\",\"method\":\"eth_getTransactionReceipt\",\"params\":[\"$TX_HASH\"],\"id\":1}")
#     echo "Receipt response: $RECEIPT_RESPONSE"
    
#     # Contract address is in logs[0].address in the receipt
...
#     if [ -z "$CONTRACT_ADDRESS" ] || [ "$CONTRACT_ADDRESS" = "null" ] || [ "$CONTRACT_ADDRESS" = "empty" ]; then
#         CONTRACT_ADDRESS="0x0000000000000000000000000000000000000000"
#         echo "Warning: Could not extract contract address from receipt"
#     else
