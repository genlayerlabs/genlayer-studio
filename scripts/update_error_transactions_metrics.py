...
def main():
    ...
    for batch in fetch_error_transactions(
        ...
        if args.dry_run:
            ...
            print(
                f"[DRY RUN] Would send {len(decisions)} decisions (total matched: {total_matched})"
            )
        else:
    ...
    print("\n" + "=" * 50)
    print("Summary:")
    print(f"  Total matched (error/timeout/undetermined): {total_matched}")
    if args.dry_run:
